...
new Vue({
    ...
    created() {
        ...
        window.addEventListener('message', (event) => {
            if (event.data && event.data.type === 'modelSelected') {
                this.loadModelAttributes(event.data.modelId);
            }
        });
        // 定期检查模型选择变化
        setInterval(() => {
    ...
    methods: {
        ...
        async loadModelAttributes(modelId) {
            try {
                // 如果没有提供modelId，从localStorage获取
                if (!modelId) {
                ...
                // 获取模型详情
                const response = await fetch('/api/models/detail', {
                    method: 'POST',
                    headers: {
                ...
                if (result.code === 200 && result.data && result.data.attributes) {
                    const attrs = result.data.attributes;
                    // 更新属性
                    this.attribute_name = attrs.name || '';
                    this.attribute_gender = attrs.gender || '';
                    this.attribute_age = attrs.age || '';
                    this.attribute_birth = attrs.birth || '';
                    this.attribute_zodiac = attrs.zodiac || '';
                    this.attribute_constellation = attrs.constellation || '';
                    this.attribute_job = attrs.job || '';
                    this.attribute_additional = attrs.additional || '';
                    this.attribute_contact = attrs.contact || '';
                    this.attribute_voice = attrs.voice || 'abin';
                    this.attribute_position = attrs.position || '客服';
                    this.attribute_goal = attrs.goal || '解决问题';
                    this.sendSuccessMsg('已加载模型属性');
                }
            } catch (error) {
        ...
        checkModelSelection() {
            // 定期检查localStorage中的模型选择变化
            let lastCheck = localStorage.getItem('modelSelected');
            if (lastCheck) {
                try {
                    const data = JSON.parse(lastCheck);
                    // 如果时间戳在5秒内，说明是新的选择
                    if (Date.now() - data.timestamp < 5000) {
                        this.loadModelAttributes(data.modelId);
                    }
                } catch (e) {
                    // 忽略解析错误
                }
