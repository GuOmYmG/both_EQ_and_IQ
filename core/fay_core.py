...
class FeiFei:
            ...
            try:
                ...
                if index == 1: #语音、文字交互
                    ...
                    try:
                        ...
                        util.log(1, f"[对话] 获取用户模型失败: {e}")
                    
                    #记录用户问题（包含模型ID）
                    content_id = content_db.new_instance().add_content('member','speak',interact.data["msg"], username, uid, model_id)
                    ...
                    if answer is None or type != "qa":
                        ...
                        if wsa_server.get_instance().is_connected(username):
                            ...
                            wsa_server.get_instance().add_cmd(content)
                        text = nlp_cognitive_stream.question(interact.data["msg"], username, interact.data.get("observation", None), interact.data.get("pure_mode", False))
    ...
    #获取不同情绪声音
    def __get_mood_voice(self, username=None):
        """
        获取声音类型，优先从用户当前选择的模型获取
        ...
        if username:
            try:
                ...
                if model_id:
                    ...
                    if attributes and attributes.get("voice"):
                        voice_name = attributes["voice"]
            except Exception as e:
                util.log(1, f"[获取声音] 从模型获取voice失败: {e}，使用config.json")
        
        # 如果无法从模型获取，使用config.json
        if voice_name == "abin":
            try:
                cfg.load_config()
                voice_name = cfg.config.get("attribute", {}).get("voice", "abin")
            except Exception:
                pass
        
        voice = tts_voice.get_voice_of(voice_name)
        if voice is None:
    ...
    def say(self, interact, text, type = ""):
        try:
            ...
            if is_first == True:
                ...
                conv_no = 0
                # 创建第一条数据库记录，获得content_id（包含模型ID）
                if text and text.strip():
                    content_id = content_db.new_instance().add_content('fay', 'speak', text, username, uid, model_id)
                else:
                    content_id = content_db.new_instance().add_content('fay', 'speak', '', username, uid, model_id)
            ...
            if audio_url is not None:#透传音频下载
                ...
                if text != None and text.replace("*", "").strip() != "":
                    ...
                    if filtered_text is not None and filtered_text.strip() != "":
                        ...
                        tm = time.time()
                        result = self.sp.to_sample(filtered_text, self.__get_mood_voice(interact.data.get('user')))
    ...
    def __record_response(self, text, username, uid, model_id=None):
        """
        记录AI的回复内容
        :param text: 回复文本
        :param username: 用户名
        ...
        return content_db.new_instance().add_content('fay', 'speak', text, username, uid, model_id)
...
# 延迟导入 fay_booter 以避免循环导入
# fay_booter 将在需要时在函数内部导入
