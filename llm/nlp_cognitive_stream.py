...
请结合以上信息，为用户生成自然语言答复，保持人设与语气一致。
    ...
    return [
        SystemMessage(content="你是最终回复的口播助手，请用中文自然表达。重要：请保持回复简短，控制在1-3句话内，像真实的人与人之间的对话一样自然简洁。"),
        HumanMessage(content=user_block),
    ]
...
def _load_attributes_from_config(agent):
    ...
    try:
        ...
        scratch_data = {
            ...
            "voice": cfg.config.get("attribute", {}).get("voice", "abin"),
...
def question(content, username, observation=None, pure_mode=False):
    """处理用户提问并返回回复。"""
    ...
    if pure_mode:
        ...
        if character_name in ['孔子', '老子', '庄子', '孟子', '荀子', '墨子', '韩非子', '李白', '杜甫', '苏轼', '李清照', '曹操', '刘备', '诸葛亮']:
            ...
            system_prompt = (
                ...
                f"重要：请保持回复简短，控制在1-3句话内，像真实的人与人之间的对话一样自然简洁。\n\n"
            )
        else:
            # 普通数字人助理的系统提示
            system_prompt = (
                ...
                "重要：请保持回复简短，控制在1-3句话内，像真实的人与人之间的对话一样自然简洁。\n\n"
            )
