...
class FayInterface {
  ...
  getMessageHistory(username, modelId) {
    return new Promise((resolve, reject) => {
      const url = `${this.baseApiUrl}/api/get-msg`;
      const xhr = new XMLHttpRequest();
      xhr.open("POST", url);
      xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
      const send_data = `data=${encodeURIComponent(JSON.stringify({ username, model_id: modelId }))}`;
      xhr.send(send_data);
...
new Vue({
  ...
  methods: {
    ...
    sendMessage() {
      ...
      // 在纯净模式下，立即添加用户消息到界面
      if (this.pure_mode_enabled) {
        this.addMessageToChat('member', text, usernameToSend);
      }
      ...
      if (this.pure_mode_enabled) {
        // 纯净LLM模式：调用直接LLM API
        console.log('使用纯净AI模式');
        
        fetch(`${this.base_url}/api/direct-llm`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        .then(data => {
          if (data.success) {
            this.addMessageToChat('fay', data.reply, usernameToSend);
          } else {
            this.addMessageToChat('fay', '抱歉，出现了错误：' + (data.error || '未知错误'), usernameToSend);
          }
        })
        .catch(error => {
          console.error('纯净AI模式请求失败:', error);
          this.addMessageToChat('fay', '网络错误，请稍后重试', usernameToSend);
        });
      } else {
        // Fay模式：调用原有API
        console.log('使用Fay模式');
        ...
        xhr.open("post", `${this.base_url}/api/send`);
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.send('data=' + encodeURIComponent(JSON.stringify(send_data)));
        
        let executed = false;
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4 && !executed) {
            executed = true;
            if (xhr.status === 200) {
              try {
                const response = JSON.parse(xhr.responseText);
                // Fay模式通过WebSocket接收回复，这里不需要处理
              } catch (e) {
                console.error('Fay模式响应解析错误:', e);
              }
            } else {
              console.error('Fay模式HTTP错误:', xhr.status);
            }
          }
        };
      }
  ...
  togglePureMode(){
    if(this.pure_mode_enabled){
      this.pure_mode_enabled = false
    }else{
      this.pure_mode_enabled = true
    }
    // 显示切换提示
    const mode = this.pure_mode_enabled ? '纯净AI助手' : '数字人角色';
    this.$message({
  ...
  addMessageToChat(type, content, username) {
    // 为纯净LLM模式添加消息到聊天界面
    ...
    const seconds = date.getSeconds().toString().padStart(2, '0');
    const milliseconds = date.getMilliseconds().toString().padStart(3, '0');
    const timetext = `${year}-${month}-${day} ${hours}:${minutes}:${seconds}.${milliseconds}`;
  ...
  stopLive() {
    ...
    loadMessageHistory(username, type) {
      ...
      try {
        ...
        console.error('获取模型ID失败:', e);
      }
      
      this.fayService.getMessageHistory(username, modelId).then((response) => {
        if (response) {
    ...
    watchModelSelection() {
      // 定期检查模型选择变化
      let lastModelId = localStorage.getItem('selectedModelId');
      setInterval(() => {
        try {
          ...
          if (currentModelId !== lastModelId) {
            ...
            // 模型选择变化，重新加载对话记录
            if (this.selectedUser) {
              this.loadMessageHistory(this.selectedUser[1], 'common');
            }
          }
        } catch (e) {
          console.error('检查模型选择失败:', e);
        }
      }, 1000);
      
      // 监听localStorage变化（跨标签页）
      window.addEventListener('storage', (e) => {
        if (e.key === 'selectedModelId' || e.key === 'modelSelected') {
          if (this.selectedUser) {
            this.loadMessageHistory(this.selectedUser[1], 'common');
          }
        }
      ...
      // 监听postMessage（同页面内通信）
      window.addEventListener('message', (event) => {
        if (event.data && event.data.type === 'modelSelected') {
          if (this.selectedUser) {
            this.loadMessageHistory(this.selectedUser[1], 'common');
        }
        }
